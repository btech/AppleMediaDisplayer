...
open class PagingMenuController: UIViewController {
    ...
    public fileprivate(set) var menuView: MenuView? {
        ...
        didSet {
            ...
            menuView.delegate = self; menuView.onMove = onMove
            menuView.update(currentPage: traits.defaultPage)
    ...
    fileprivate var traits: PagingMenuControllerCustomizable {
        didSet {
            cleanup()
    ...
    override open func viewWillTransition(to size: CGSize, with coordinator: UIViewControllerTransitionCoordinator) {
        ...
        if let menuView = menuView {
            ...
            coordinator.animate(alongsideTransition: { [unowned self] (_) -> Void in
                ...
                switch menuView.options.displayMode {
    ...
    open func setup() {
        ...
        menuView = MenuView(options: traits.menuTraits)
        addTapGestureHandler(); addSwipeGestureHandler(); layoutMenuView()
    ...
    fileprivate func setupMenuController() {
        ...
        pagingViewController = PagingViewController(viewControllers: traits.pagingControllers, options: traits)
        ...
        switch menuView!.options.menuPosition {
    ...
    open func move(toPage page: Int, animated: Bool = true) {
        ...
        let duration = animated ? traits.animationDuration : 0
        let animationClosure = { pagingViewController.positionMenuController() }
        ...
        } else { animationClosure(); completionClosure(true) }
    }
    ...
    fileprivate func layoutMenuView() {
        ...
        NSLayoutConstraint.activate([
            ...
            menuView.heightAnchor.constraint(equalToConstant: menuView.options.height)
            ])
        
        menuView.setNeedsLayout(); menuView.layoutIfNeeded()
    }
    ...
    fileprivate func hidePagingMenuControllers(_ page: Int) {
        ...
        switch (traits.lazyLoadingPage, menuView!.options.displayMode, page) {
            
        case (.three, .infinite, menuView!.previousPage),
             (.three, .infinite, menuView!.nextPage):
    ...
    public init(traits: PagingMenuControllerCustomizable) {
...
extension PagingMenuController: Pagable {
    ...
    public var currentPage: Int {
        ...
        case (let menuView?, _): return menuView.currentPage
            
        case (_, let pagingViewController?): return pagingViewController.currentPage
    ...
    var previousPage: Int {
        ...
        guard let menuOptions = menuView?.options,
            case .infinite = menuOptions.displayMode,
            let controllers = pagingViewController?.controllers
...
extension PagingMenuController {
    ...
    fileprivate var currentPagingViewPosition: PagingViewPosition {
        ...
        if let menuOptions = menuView?.options,
            case .infinite = menuOptions.displayMode {
    ...
    fileprivate var nextPageFromCurrentPosition: Int {
        ...
        // return page number according to current moving direction
        switch traits.lazyLoadingPage {
        case .all:
            
            guard let scrollView = pagingViewController?.contentScrollView else { return currentPage }
            
            return Int(scrollView.contentOffset.x) / Int(scrollView.frame.width)
            
        default:
            
            switch (currentPagingViewPosition) {
                
            case .left: return menuView?.previousPage ?? previousPage
                
            case .right: return menuView?.nextPage ?? nextPage
...
extension PagingMenuController {
    ...
    fileprivate func addSwipeGestureHandler() {
        ...
        guard let menuOptions = menuView?.options else { return }
    ...
    @objc internal func handleSwipeGesture(_ recognizer: UISwipeGestureRecognizer) {
        ...
        guard let menuView = recognizer.view as? MenuView,
            let menuOptions = self.menuView?.options else { return }
        ...
        case (UISwipeGestureRecognizerDirection.left, _):
            newPage = min(nextPage, menuOptions.itemTraits.count - 1)
...
extension PagingMenuController {
    ...
    fileprivate func validateContentsCount(_ options: PagingMenuControllerCustomizable) {
        ...
        guard menuView!.options.itemTraits.count == options.pagingControllers.count else {
    ...
    fileprivate func validateDefaultPage(_ options: PagingMenuControllerCustomizable) {
        ...
        guard options.defaultPage >= options.pagingControllers.count || options.defaultPage < 0 else { return }
    ...
    fileprivate func validateInfiniteMenuItemNumbers(_ options: PagingMenuControllerCustomizable) {
        ...
        guard case .infinite = menuView!.options.displayMode else { return }
        guard menuView!.options.itemTraits.count < VisiblePagingViewNumber else { return }

...
open class PagingViewController: UIViewController {
    ...
    fileprivate func constructPagingViewControllers() {
        ...
        for (index, controller) in controllers.enumerated() {
            ...
            if isVisible(controller: controller) { continue }
            
            guard let pagingView = controller.view else { fatalError("\(controller) doesn't have any view") }
            ...
            // TODO: Temporary fix to prevent initially invisible, nested PagingMenuController's from not loading properly
                if let pagingController = controller as? PagingMenuController {

                    pagingController.pagingViewController?.relayoutPagingViewControllers()
                }
    ...
    fileprivate func layoutPagingViewControllers() {
        ...
        for (index, controller) in controllers.enumerated() {
            ...
            options.menuControllerSet == MenuControllerSet.single {
                ...
                if case .infinite = options.menuTraits.displayMode {
    ...
    internal func positionMenuController() {
        ...
        contentScrollView.contentOffset.x = (!isInitialSetup || currentView.frame.minX != 0) ? currentView.frame.minX : UIScreen.main.bounds.width
        }
    }
}
...
extension PagingViewController: Pagable {
    ...
    public var currentPage: Int { return currentIndex }
    
    var previousPage: Int {
        
        guard case .infinite = options.menuTraits.displayMode else { return currentPage - 1 }
        
        return ((currentPage - 1) < 0) ? (options.menuTraits.itemTraits.count - 1) : (currentPage - 1)
    }
    ...
    var nextPage: Int {
        ...
        return ((currentPage + 1) > (options.menuTraits.itemTraits.count - 1)) ? 0 : (currentPage + 1)
    }
    
    func update(currentPage page: Int) { currentIndex = page }
}
...
extension PagingViewController {
    ...
    fileprivate func shouldWaitForLayout() -> Bool {
        ...
        guard case .infinite = options.menuTraits.displayMode else { return false }

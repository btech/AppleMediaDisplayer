...
open class MenuItemView: UIView {
    ...
    lazy public var menuImageView: UIImageView = {
        ...
        $0.isUserInteractionEnabled = true; $0.translatesAutoresizingMaskIntoConstraints = false
    ...
    public fileprivate(set) var customView: UIView? {
        ...
        didSet {
            ...
            guard let customView = customView else { return }; addSubview(customView)
        }
    }
    ...
    public internal(set) var isSelected: Bool = false {
        ...
        didSet {
            ...
            if case .roundRect = menuOptions.focusMode { backgroundColor = UIColor.clear }
            else {
    ...
    internal init(menuOptions: MenuViewCustomizable,
                  menuItemOptions: MenuItemViewCustomizable,
                  addDivider: Bool) {
    ...
    internal func updateConstraints(_ size: CGSize) {
        ...
        case .image, .custom:
            ...
            widthConstraint.constant = size.width / CGFloat(menuOptions.itemTraits.count)
        }
    }
    ...
    fileprivate func setupView() {
        ...
        if case .roundRect = menuOptions.focusMode { backgroundColor = UIColor.clear }
        else { backgroundColor = menuOptions.backgroundColor }
        
        translatesAutoresizingMaskIntoConstraints = false
    }
    
    fileprivate func setupTitleLabel(_ text: MenuItemText) { setupLabel(titleLabel, text: text) }
    
    fileprivate func setupMultilineLabel(_ text: MenuItemText, description: MenuItemText) {
        
        setupLabel(titleLabel, text: text); setupLabel(descriptionLabel, text: description)
    }
    
    fileprivate func setupLabel(_ label: UILabel, text: MenuItemText) {
        
        label.text = text.text; updateLabel(label, text: text); addSubview(label)
    }
    ...
    fileprivate func setupImageView(_ image: UIImage) {
        ...
        menuImageView.image = image; addSubview(menuImageView)
    }
    
    fileprivate func setupCustomView(_ view: UIView) { customView = view }
    
    fileprivate func setupDivider() {
        
        guard let dividerImageView = dividerImageView else { return }; addSubview(dividerImageView)
    }
    ...
    fileprivate func layoutImageView() {
        ...
        case .segmentedControl:
            ...
            if let windowWidth = UIApplication.shared.keyWindow?.bounds.size.width {
                ...
                width = windowWidth / CGFloat(menuOptions.itemTraits.count)
                
            } else {
                
                width = UIScreen.main.bounds.width / CGFloat(menuOptions.itemTraits.count)
            }
            
        default: width = image.size.width + horizontalMargin * 2
        }
...
extension MenuItemView {
    ...
    func cleanup() {
        ...
        case .multilineText: titleLabel.removeFromSuperview(); descriptionLabel.removeFromSuperview()
            
        case .image:         menuImageView.removeFromSuperview()
            
        case .custom:        customView?.removeFromSuperview()
        }
...
extension MenuItemView {
    ...
    fileprivate func estimatedLabelSize(_ label: UILabel) -> CGSize {
        ...
        return NSString(string: text).boundingRect(with: CGSize(width: CGFloat.greatestFiniteMagnitude,
    ...
    fileprivate func calculateLabelSize(_ label: UILabel, maxWidth: CGFloat) -> CGSize {
        ...
        guard !label.text.isNil else { return .zero }
        ...
        let itemWidth: CGFloat = {
            ...
            case .standard(let widthMode, _, _):
                ...
                return labelWidth(widthMode, estimatedSize: estimatedLabelSize(label))
                
            case .segmentedControl:
                
                return maxWidth / CGFloat(menuOptions.itemTraits.count)
